function [data_cell, A, dim, u, b] = generate_nqp_data_exp(dim, num_agents, batch_size, num_constraints, data_mean_value, constraint_mean_value)
...
for i = 1 : num_agents
    ...
    for j = 1 : batch_size
        ...
        rand_mean = rand() * abs(data_mean_value);
        tmp_cell{j} = - exprnd(rand_mean, [dim, dim]);
    end
    data_cell{i} = tmp_cell;
end

% A = rand(num_constraints, dim);
A = exprnd(abs(constraint_mean_value), [num_constraints, dim]);
u = ones(dim, 1);
